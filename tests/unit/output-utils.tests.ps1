...
BeforeAll {
    ...
    Import-Module (Join-Path $PSScriptRoot '../../scripts/lib/Logging.psm1') -Force
...
Describe 'OutputUtils Module Tests' {
    Context 'ConvertTo-RepoRelativePath' {
        It 'Converts absolute paths to relative' {
            $absolutePath = Join-Path $script:TestRepoRoot 'tests/unit/common.tests.ps1'
            $relativePath = ConvertTo-RepoRelativePath -PathString $absolutePath

            $relativePath -replace '\\', '/' | Should -Be 'tests/unit/common.tests.ps1'
        }
