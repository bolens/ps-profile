...
Describe 'check-missing-packages.ps1 execution' {
    ...
    It 'Passes when isolated manifests declare no dependencies to check' {
        $repo = New-TestTempDirectory -Prefix 'CheckMissingPackagesEmpty'
        try {
            $scriptDir = Join-Path $repo 'scripts' 'utils' 'dependencies'
            $null = New-Item -ItemType Directory -Path $scriptDir -Force
            Copy-Item -LiteralPath (Join-Path $script:TestRepoRoot 'scripts' 'lib') -Destination (Join-Path $repo 'scripts' 'lib') -Recurse -Force
            ...
            @{
                name    = 'missing-packages-empty-fixture'
                version = '1.0.0'
            } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $repo 'package.json') -Encoding UTF8
            Set-Content -LiteralPath (Join-Path $repo 'requirements.txt') -Value '' -Encoding UTF8

            $requirementsDir = Join-Path $repo 'requirements'
            $null = New-Item -ItemType Directory -Path $requirementsDir -Force
            @(
                '# --- apt (Debian/Ubuntu) ---'
                '# --- pacman (Arch / CachyOS / Manjaro) ---'
                '# --- dnf (Fedora / RHEL / Rocky / Alma) ---'
            ) | Set-Content -LiteralPath (Join-Path $requirementsDir 'linux.txt') -Encoding UTF8
