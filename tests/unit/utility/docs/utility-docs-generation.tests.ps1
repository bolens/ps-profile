...
'@
        ...
        It 'renders example sections with valid markdown code fences' {
            $tempDir = Join-Path $script:TestTempRoot 'docs_example_fence'
            New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
...
<#
.SYNOPSIS
    Example fence test function.
.DESCRIPTION
    Verifies generated example markdown uses proper fenced code blocks.
.OUTPUTS
    System.String
    Returns the output path.
.EXAMPLE
    Test-ExampleFence -InputPath 'data.txt'
    Runs the example against data.txt.
#>
function Test-ExampleFence {
...
'@
            ...
            $functionDocPath = Join-Path $outputPath 'functions' 'Test-ExampleFence.md'
            Test-Path $functionDocPath | Should -Be $true

            $content = Get-Content $functionDocPath -Raw
            $content | Should -Match '```powershell'
            $content | Should -Not -Match '(?m)^`powershell'
            $content | Should -Match 'Test-ExampleFence -InputPath ''data.txt'''
            $content | Should -Match 'Runs the example against data.txt'
            $content | Should -Not -Match '\.EXAMPLE'
        }
