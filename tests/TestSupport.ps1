...
<#
.SYNOPSIS
    Imports the shared Common module for test runs.
.DESCRIPTION
    Ensures the Common.psm1 module is loaded once per session and returns its path.
.OUTPUTS
    System.String
#>
function Import-TestCommonModule {
    $modulePath = Get-TestPath -RelativePath 'scripts\lib\Common.psm1' -StartPath $PSScriptRoot -EnsureExists
    $alreadyLoaded = Get-Module | Where-Object { $_.Path -eq $modulePath }
...
function New-TestTempDirectory {
    ...
    $path = Join-Path $tempRoot $uniqueName
...
function Invoke-TestPwshScript {
    ...
    $tempFile = Join-Path ([System.IO.Path]::GetTempPath()) ([System.IO.Path]::GetRandomFileName() + '.ps1')
