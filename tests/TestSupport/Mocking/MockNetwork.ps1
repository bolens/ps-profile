...
# Load mock registry and Pester mock functions (dot-sourced by TestSupport when available)
$modulePath = Split-Path -Parent $MyInvocation.MyCommand.Path
if (-not (Get-Command Register-Mock -ErrorAction SilentlyContinue)) {
    . (Join-Path $modulePath 'MockRegistry.ps1')
}
if (-not (Get-Command Mock-CommandAvailabilityPester -ErrorAction SilentlyContinue)) {
    . (Join-Path $modulePath 'PesterMocks.ps1')
}
