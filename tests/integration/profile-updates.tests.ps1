Describe 'Profile Updates Integration Tests' {
    BeforeAll {
        ...
        if (-not (Test-Path $script:TestProfileDir)) {
    ...
    AfterAll {
        ...
        if ($script:TestProfileDir -and (Test-Path $script:TestProfileDir)) {
    ...
    Context 'Test-ProfileUpdates function' {
        BeforeAll {
            ...
            . (Join-Path $script:ProfileDir '70-profile-updates.ps1')
        }
        ...
        BeforeEach {
            ...
            if (Test-Path $script:LastCheckFile) {
