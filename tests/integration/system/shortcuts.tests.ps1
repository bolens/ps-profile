...
Describe 'Shortcuts Integration Tests' {
    BeforeAll {
        try {
            ...
            Write-Error "Failed to initialize shortcuts tests in BeforeAll: $($errorDetails | ConvertTo-Json -Compress)" -ErrorAction Stop
            throw
        }
    }
    ...
    Context 'Shortcuts functions' {
        BeforeAll {
            ...
            if (Test-Path -LiteralPath "Function:\Get-AvailableEditor") {
        ...
        It 'e alias exists for Open-Editor' {
            Get-Alias e -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
            (Get-Alias e).ResolvedCommandName | Should -Be 'Open-Editor'
        }
        ...
        It 'project-root alias exists for Get-ProjectRoot' {
            Get-Alias project-root -ErrorAction SilentlyContinue | Should -Not -BeNullOrEmpty
            (Get-Alias project-root).ResolvedCommandName | Should -Be 'Get-ProjectRoot'
        }
    }
}
