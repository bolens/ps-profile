...
BeforeAll {
    try {
        ...
        Write-Error "Failed to initialize baseline comparison tests in BeforeAll: $($errorDetails | ConvertTo-Json -Compress)" -ErrorAction Stop
        throw
    }
...
Describe 'Performance Baseline Comparison Integration Tests' {
    Context 'Baseline Generation' {
        ...
        It 'Handles default baseline path correctly' {
            ...
            if ($defaultBaselinePath -and -not [string]::IsNullOrWhiteSpace($defaultBaselinePath) -and (Test-Path -LiteralPath $defaultBaselinePath)) {
    ...
    Context 'Baseline File Management' {
        ...
        It 'Overwrites existing baseline files' {
            ...
            # Wait a moment and generate again (reduced delay since timestamp check is disabled)
