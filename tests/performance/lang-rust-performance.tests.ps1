...
BeforeAll {
    ...
    $script:MaxFragmentLoadTimeMs = Get-PerformanceThreshold -EnvironmentVariable 'PS_PROFILE_LANG_RUST_MAX_LOAD_MS' -Default 2000
    $script:MaxLookupTimeMs = Get-PerformanceThreshold -EnvironmentVariable 'PS_PROFILE_LANG_RUST_MAX_LOOKUP_MS' -Default 500
    $script:MaxIdempotencyTimeMs = Get-PerformanceThreshold -EnvironmentVariable 'PS_PROFILE_LANG_RUST_MAX_IDEMPOTENCY_MS' -Default 3000
    $script:MaxVarianceMs = Get-PerformanceThreshold -EnvironmentVariable 'PS_PROFILE_LANG_RUST_MAX_VARIANCE_MS' -Default 500
