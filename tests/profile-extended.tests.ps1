Describe 'Profile Extended Coverage Tests' {
    ...
    Context 'File listing functions' {
        ...
        It 'Get-ChildItemDetailed (ll) function is available' {
            Get-Command ll -CommandType Function -ErrorAction SilentlyContinue | Should Not Be $null
        ...
        It 'Get-ChildItemAll (la) function is available' {
            Get-Command la -CommandType Function -ErrorAction SilentlyContinue | Should Not Be $null
        ...
        It 'Get-ChildItemVisible (lx) function is available' {
            Get-Command lx -CommandType Function -ErrorAction SilentlyContinue | Should Not Be $null
        ...
        It 'Get-DirectoryTree (tree) function is available' {
            Get-Command tree -CommandType Function -ErrorAction SilentlyContinue | Should Not Be $null
        ...
        It 'Show-FileContent (bat-cat) function is available' {
            Get-Command bat-cat -CommandType Function -ErrorAction SilentlyContinue | Should Not Be $null
    ...
    Context 'File navigation functions' {
        ...
        It '~ function navigates to home directory' {
            ...
            try {
                ~
                $home = Get-Location
                $home.Path | Should Match ([regex]::Escape($env:USERPROFILE))
            }
            finally {
    ...
    Context 'Utility functions edge cases' {
        ...
        It 'epoch returns consistent timestamps' {
            ...
            $time2 = epoch
            $time2 | Should BeGreaterThan $time1
        }
    ...
    Context 'Utility functions additional tests' {
        ...
        It 'Get-History returns recent commands' {
            ...
            # Should return history objects
            if ($result.Count -gt 0) {
                $result[0] | Should HaveMember 'CommandLine'
            }
        }

        It 'Find-History searches command history' {
            # Add a test command to history
            'test-search-command-12345' | Out-Host
            Start-Sleep -Milliseconds 100
            
            $result = Find-History 'test-search-command-12345'
            # Should find the command or return empty
            if ($result) {
                $result | Should Not BeNullOrEmpty
            }
        }
