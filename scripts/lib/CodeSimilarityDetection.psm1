...
$stringSimilarityModulePath = Join-Path $PSScriptRoot 'StringSimilarity.psm1'
...
if (Test-Path $stringSimilarityModulePath) {
...
function Get-CodeSimilarity {
    ...
    if ($scriptBlocks.Count -lt 2) {
        Write-Verbose "Need at least 2 scripts with blocks to compare similarity"
        return @()
    }
        ...
        for ($j = $i + 1; $j -lt $scriptPaths.Count; $j++) {
            ...
            foreach ($block1 in $blocks1) {
                foreach ($block2 in $blocks2) {
                    ...
                    # Check if Get-StringSimilarity is available, otherwise skip
                    if (-not (Get-Command Get-StringSimilarity -ErrorAction SilentlyContinue)) {
                        Write-Warning "Get-StringSimilarity function not available, skipping similarity calculation"
                        continue
                    }
                    ...
                    try {
                        ...
                        Write-Warning "Failed to calculate similarity: $($_.Exception.Message)"
                        continue
                    }
