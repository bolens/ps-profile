...
.PARAMETER GitExecutable
    Git command used for history, tagging, and pushing. Defaults to git.

.PARAMETER ChangelogScript
    Changelog generator command. Defaults to generate-changelog.ps1 beside this
    script.
...
param(
    ...
    [string]$ChangelogScript = (Join-Path $PSScriptRoot 'generate-changelog.ps1')
)
...
# Get the latest tag
$latestTag = & $GitExecutable describe --tags --abbrev=0 2>$null
if (-not $latestTag) {
...
$commits = & $GitExecutable log --pretty=format:"%s" "$latestTag..HEAD" 2>$null
...
try {
    & $GitExecutable tag -a "v$newVersion" -m "Release v$newVersion"
    if ($LASTEXITCODE -ne 0) {
...
try {
    & $GitExecutable push origin "v$newVersion"
    if ($LASTEXITCODE -ne 0) {
