...
if (Test-Path Function:\Register-LazyFunction) {
    ...
    # GitHub PR create - create a pull request
    if (-not (Test-Path Function:New-GitHubPullRequest)) { Set-Item -Path Function:New-GitHubPullRequest -Value { Ensure-GitHelper; & (Get-Command New-GitHubPullRequest -CommandType Function).ScriptBlock.InvokeReturnAsIs($args) } -Force | Out-Null; Set-Alias -Name prc -Value New-GitHubPullRequest -ErrorAction SilentlyContinue }
    # GitHub PR view - view pull request in browser
    if (-not (Test-Path Function:Show-GitHubPullRequest)) { Set-Item -Path Function:Show-GitHubPullRequest -Value { Ensure-GitHelper; & (Get-Command Show-GitHubPullRequest -CommandType Function).ScriptBlock.InvokeReturnAsIs($args) } -Force | Out-Null; Set-Alias -Name prv -Value Show-GitHubPullRequest -ErrorAction SilentlyContinue }
}
