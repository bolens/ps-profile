...
if (-not (Test-Path Function:deno -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:deno -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand deno) { deno @a } else { Write-Warning 'deno not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:deno -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command deno -ErrorAction SilentlyContinue)) { deno @a } else { Write-Warning 'deno not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:deno-run -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:deno-run -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command deno -ErrorAction SilentlyContinue)) { deno run @a } else { Write-Warning 'deno not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:deno-task -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:deno-task -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command deno -ErrorAction SilentlyContinue)) { deno task @a } else { Write-Warning 'deno not found' } } -Force | Out-Null
  }
}
