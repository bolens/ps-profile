...
if (-not (Test-Path Function:fb -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:fb -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand firebase) { firebase @a } else { Write-Warning 'firebase not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:fb -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command firebase -ErrorAction SilentlyContinue)) { firebase @a } else { Write-Warning 'firebase not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:fb-deploy -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:fb-deploy -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command firebase -ErrorAction SilentlyContinue)) { firebase deploy @a } else { Write-Warning 'firebase not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:fb-serve -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:fb-serve -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command firebase -ErrorAction SilentlyContinue)) { firebase serve @a } else { Write-Warning 'firebase not found' } } -Force | Out-Null
  }
}
