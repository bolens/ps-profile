...
if (-not (Test-Path Function:ng -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ng -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx @angular/cli @a } elseif ($null -ne (Get-Command ng -ErrorAction SilentlyContinue)) { ng @a } else { Write-Warning 'npx or ng not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:ng-new -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ng-new -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx @angular/cli new @a } elseif ($null -ne (Get-Command ng -ErrorAction SilentlyContinue)) { ng new @a } else { Write-Warning 'npx or ng not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:ng-serve -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ng-serve -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx @angular/cli serve @a } elseif ($null -ne (Get-Command ng -ErrorAction SilentlyContinue)) { ng serve @a } else { Write-Warning 'npx or ng not found' } } -Force | Out-Null
  }
}
