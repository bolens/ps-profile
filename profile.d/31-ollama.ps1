...
if (-not (Test-Path Function:ol -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:ol -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand ollama) { ollama @a } else { Write-Warning 'ollama not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:ol -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command ollama -ErrorAction SilentlyContinue)) { ollama @a } else { Write-Warning 'ollama not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:ol-list -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ol-list -Value { if ($null -ne (Get-Command ollama -ErrorAction SilentlyContinue)) { ollama list } else { Write-Warning 'ollama not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:ol-run -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ol-run -Value { param($model) if ($null -ne (Get-Command ollama -ErrorAction SilentlyContinue)) { ollama run $model } else { Write-Warning 'ollama not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:ol-pull -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ol-pull -Value { param($model) if ($null -ne (Get-Command ollama -ErrorAction SilentlyContinue)) { ollama pull $model } else { Write-Warning 'ollama not found' } } -Force | Out-Null
  }
}
