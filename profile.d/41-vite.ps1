...
if (-not (Test-Path Function:vite -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:vite -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand npx) { npx vite @a } else { Write-Warning 'npx not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:vite -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx vite @a } else { Write-Warning 'npx not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:create-vite -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:create-vite -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx create-vite @a } else { Write-Warning 'npx not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:vite-dev -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:vite-dev -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx vite dev @a } else { Write-Warning 'npx not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:vite-build -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:vite-build -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command npx -ErrorAction SilentlyContinue)) { npx vite build @a } else { Write-Warning 'npx not found' } } -Force | Out-Null
  }
}
