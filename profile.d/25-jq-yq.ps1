...
if (-not (Test-Path Function:jq2json -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:jq2json -Value { param($f) if ($null -ne (Get-Command jq -ErrorAction SilentlyContinue)) { jq -c . $f } else { Write-Warning 'jq not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:yq2json -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:yq2json -Value { param($f) if ($null -ne (Get-Command yq -ErrorAction SilentlyContinue)) { yq eval -o=json $f } else { Write-Warning 'yq not found' } } -Force | Out-Null
  }
}
