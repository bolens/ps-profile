...
if (-not (Test-Path Function:ff -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:ff -Value { param([string]$pattern = '') if ($null -ne (Get-Command fzf -ErrorAction SilentlyContinue)) { Get-ChildItem -Recurse -File | Where-Object { $_.Name -match $pattern } | fzf } else { Write-Warning 'fzf not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:fcmd -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:fcmd -Value { if ($null -ne (Get-Command fzf -ErrorAction SilentlyContinue)) { Get-Command | Out-String | fzf } else { Write-Warning 'fzf not found' } } -Force | Out-Null
  }
}
