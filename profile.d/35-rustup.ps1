...
if (-not (Test-Path Function:rustup -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:rustup -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand rustup) { rustup @a } else { Write-Warning 'rustup not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:rustup -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command rustup -ErrorAction SilentlyContinue)) { rustup @a } else { Write-Warning 'rustup not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:rustup-update -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:rustup-update -Value { if ($null -ne (Get-Command rustup -ErrorAction SilentlyContinue)) { rustup update } else { Write-Warning 'rustup not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:rustup-install -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:rustup-install -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command rustup -ErrorAction SilentlyContinue)) { rustup install @a } else { Write-Warning 'rustup not found' } } -Force | Out-Null
  }
}
