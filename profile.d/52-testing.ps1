...
if (-not (Test-Path Function:jest -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:jest -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command jest -ErrorAction SilentlyContinue)) { jest @a } else { npx jest @a } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:vitest -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:vitest -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command vitest -ErrorAction SilentlyContinue)) { vitest @a } else { npx vitest @a } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:playwright -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:playwright -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command playwright -ErrorAction SilentlyContinue)) { playwright @a } else { npx playwright @a } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:cypress -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:cypress -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command cypress -ErrorAction SilentlyContinue)) { cypress @a } else { npx cypress @a } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:mocha -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:mocha -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command mocha -ErrorAction SilentlyContinue)) { mocha @a } else { npx mocha @a } } -Force | Out-Null
  }
}
