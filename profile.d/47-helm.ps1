...
if (-not (Test-Path Function:helm -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:helm -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand helm) { helm @a } else { Write-Warning 'Helm not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:helm -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command helm -ErrorAction SilentlyContinue)) { helm @a } else { Write-Warning 'Helm not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:helm-install -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:helm-install -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command helm -ErrorAction SilentlyContinue)) { helm install @a } else { Write-Warning 'Helm not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:helm-upgrade -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:helm-upgrade -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command helm -ErrorAction SilentlyContinue)) { helm upgrade @a } else { Write-Warning 'Helm not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:helm-list -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:helm-list -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command helm -ErrorAction SilentlyContinue)) { helm list @a } else { Write-Warning 'Helm not found' } } -Force | Out-Null
  }
}
