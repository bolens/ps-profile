...
if (-not (Test-Path Function:aws -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    Set-Item -Path Function:aws -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if (Test-CachedCommand aws) { aws @a } else { Write-Warning 'aws not found' } } -Force | Out-Null
  } else {
    Set-Item -Path Function:aws -Value { param([Parameter(ValueFromRemainingArguments = $true)] $a) if ($null -ne (Get-Command aws -ErrorAction SilentlyContinue)) { aws @a } else { Write-Warning 'aws not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:aws-profile -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:aws-profile -Value { param($p) if ($null -ne (Get-Command aws -ErrorAction SilentlyContinue)) { $env:AWS_PROFILE = $p; Write-Host "AWS profile set to: $p" } else { Write-Warning 'aws not found' } } -Force | Out-Null
  }
}
...
if (-not (Test-Path Function:aws-region -ErrorAction SilentlyContinue)) {
  if (Test-Path Function:Test-CachedCommand -ErrorAction SilentlyContinue) {
    ...
    Set-Item -Path Function:aws-region -Value { param($r) if ($null -ne (Get-Command aws -ErrorAction SilentlyContinue)) { $env:AWS_REGION = $r; Write-Host "AWS region set to: $r" } else { Write-Warning 'aws not found' } } -Force | Out-Null
  }
}
