# dcu

## Synopsis

Starts container services using compose (Docker-first).

## Description

Runs 'compose up -d' using the available container engine, preferring Docker over Podman. Automatically detects and uses docker compose, docker-compose, podman compose, or podman-compose.

## Alias Information

**Alias for:** `Start-ContainerCompose`

This is an alias that points to the `Start-ContainerCompose` command. Use this alias as a shorthand for the full command name.

## Examples

No examples provided.

## Source

Defined in: profile.d\22-containers.ps1
