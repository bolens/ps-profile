...
System.String File path as a string. .OUTPUTS System.String Human-readable file size with unit. .NOTES File sizes are displayed with 2 decimal places for larger units. Uses the file's Length property from Get-Item.


## Outputs

System.String Human-readable file size with unit.
...
4.25 GB Shows the size of a large ISO file in GB.
...
2.34 KB Shows the size of a PowerShell script using the alias.
...
145 bytes Shows the size of a small text file in bytes.
...
Get-FileSize : File not found: nonexistent.txt At line:1 char:1 Shows error when file doesn't exist.
